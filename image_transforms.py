...
def get_transform(is_validation):
...
def resize(vid, size, interpolation='bilinear'):
    ...
    if isinstance(size, int):
        ...
        size = None
    length, channels, height, width = vid.shape
    vid = torch.nn.functional.interpolate(
        vid.view(-1, *vid.shape[1:]).transpose(0, -1), size=size, scale_factor=scale).transpose(0, -1)
    vid = vid.view(length, channels, *vid.shape[2:])
...
def normalize(vid, mean, std):
    shape = [1, 1, 1, 3]
    mean = torch.as_tensor(mean).reshape(shape).repeat(*vid.shape[:-1], 1)
    std = torch.as_tensor(std).reshape(shape).repeat(*vid.shape[:-1], 1)
    return (vid - mean) / std
